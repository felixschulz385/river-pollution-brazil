...
from src.data.sources.climate.core import Climate
from src.data.sources.climate.fetch.common import ClimateCredentialsError, ERA5_YEARS, load_cds_credentials
from src.data.sources.climate.fetch.common import (
    load_download_manifest,
    manifest_path_for,
    retrieve_batched_dataset,
)
from src.data.sources.climate.fetch.era5_land_daily import (
...
from src.data.sources.climate.fetch.verify import ERA5L_VALUE_RANGES, VerificationResult, verify_era5_grib_batch
...
def disable_climate_decency_waits(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr("src.data.sources.climate.fetch.common._decency_wait", lambda *args, **kwargs: None)
    monkeypatch.setattr("src.data.sources.climate.fetch.common._worker_wait", lambda *args, **kwargs: None)
    monkeypatch.setattr("src.data.sources.climate.fetch.common.ENABLE_PERIODIC_RECHECKS", False)
    monkeypatch.setattr("src.data.sources.climate.fetch.common.MAX_ACTIVE_REMOTE_REQUESTS", TOTAL_MONTHLY_BATCHES + 10)
...
def test_fetch_era5_land_daily_resumes_and_downloads_successful_job(
    ...
    monkeypatch.setattr(
        "src.data.sources.climate.fetch.era5_land_daily.verify_era5_grib_batch",
...
def test_climate_fetch_routes_supported_subtypes(
    ...
    monkeypatch.setattr(
        "src.data.sources.climate.fetch.era5_land_hourly.fetch_era5_land_hourly",
        lambda root_dir=".": hourly_output,
    )
    monkeypatch.setattr(
        "src.data.sources.climate.fetch.era5_land_daily.fetch_era5_land_daily",
        lambda root_dir=".": daily_output,
    )
    monkeypatch.setattr(
        "src.data.sources.climate.fetch.era5_land_arco.fetch_era5_land_arco",
...
def test_climate_fetch_era5_land_arco_does_the_real_work(
    ...
    monkeypatch.setattr(
        "src.data.sources.climate.preprocess.era5_land_arco.preprocess_era5_land_arco",
        lambda root_dir=".": arco_output,
    )
...
def test_verify_era5_grib_batch_passes_for_reasonable_values(
    ...
    monkeypatch.setattr("src.data.sources.climate.preprocess.era5_land._open_era5_dataset", lambda path: dataset)
...
def test_retrieve_batched_dataset_reschedules_then_caps_verification_failures(
    ...
    monkeypatch.setattr("src.data.sources.climate.fetch.common._manifest_is_due_for_remote_check", lambda manifest: True)
